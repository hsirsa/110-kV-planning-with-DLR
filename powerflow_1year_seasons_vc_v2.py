...
def reset_controlled_subnet_sgen_q_mvar(net, controlled_hv_bus_indices):
    if net.sgen.empty:
        return 0
    if "q_mvar" not in net.sgen.columns:
        net.sgen["q_mvar"] = 0.0
    controlled_set = set(int(idx) for idx in controlled_hv_bus_indices)
    controlled_sgen_indices = [
        idx for idx in net.sgen.index if int(net.sgen.at[idx, "bus"]) in controlled_set
    ]
    if controlled_sgen_indices:
        net.sgen.loc[controlled_sgen_indices, "q_mvar"] = 0.0
    return len(controlled_sgen_indices)
...
def reset_controlled_oltc_taps_to_neutral(net, controlled_hv_bus_indices):
    trafo_indices = get_controlled_oltc_trafo_indices(net, controlled_hv_bus_indices)
    for trafo_idx in trafo_indices:
        neutral_tap = net.trafo.at[trafo_idx, "tap_neutral"] if "tap_neutral" in net.trafo.columns else 0.0
        if pd.isna(neutral_tap):
            neutral_tap = 0.0
        net.trafo.at[trafo_idx, "tap_pos"] = int(round(float(neutral_tap)))
    return len(trafo_indices)
...
def voltage_controlled_runpp(net, **kwargs):
    controlled_hv_bus_indices = get_controlled_hv_bus_indices(net)
    reset_controlled_subnet_sgen_q_mvar(net, controlled_hv_bus_indices)
    reset_controlled_oltc_taps_to_neutral(net, controlled_hv_bus_indices)
...
def run_season_study(season_name, time_steps, output_root, time_lookup):
    ...
    validate_nonzero_exported_results(output_path)
