...
from .subnet import classify_subnet_bus
...
def _classify_sgen_generation_type_vectorized(sgen_df):
    ...
    combined = sgen_df[candidate_cols].fillna("").astype(str).apply(lambda row: " ".join(row.str.lower()), axis=1)
...
def build_bus_time_series_from_elements(net, abs_vals, time_steps, element_type, subnet_bus_indices):
    ...
    merged = long_df.merge(
        meta[["element_index", "bus_index", "bus_name", "generation_type"]], on="element_index", how="left"
    )
...
def build_subnet_bus_generation_mix(net, abs_vals, time_steps, subnet_bus_indices):
    ...
    summary_df = time_series_df.groupby(["bus_index", "bus_name"], as_index=False).agg(
...
def aggregate_abs_values_by_bus(net, abs_vals, time_steps, element_type, bus_column, subnet_bus_indices, source_label):
    ...
    p_df.columns = [int(col) for col in p_df.columns]
    p_long = p_df.reset_index(names="time_step").melt(
    ...
    if key_q in abs_vals:
        ...
        q_long = q_df.reset_index(names="time_step").melt(
            id_vars=["time_step"], var_name="element_index", value_name="q_mvar"
        )
...
def build_subnet_injection_timeseries(net, abs_vals, time_steps, subnet_bus_indices):
    frames = []
    for element_type in ("load", "sgen", "gen"):
        frame = aggregate_abs_values_by_bus(
            net, abs_vals, time_steps, element_type, "bus", subnet_bus_indices, element_type
        )
        if not frame.empty:
            frames.append(frame)
    if not frames:
...
def build_subnet_generation_analysis(injection_df, bus_vm):
    ...
    max_vm = bus_vm.groupby("time_step", as_index=False)["vm_pu"].max().rename(columns={"vm_pu": "max_vm_pu"})
